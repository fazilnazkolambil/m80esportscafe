/* eslint-disable */
// this is an auto generated file. This will be overwritten

export const getCurrentUserDetails = /* GraphQL */ `
  query GetCurrentUserDetails($input: GetCurrentUserDetailsInput) {
    getCurrentUserDetails(input: $input)
  }
`;
export const listOrganisations = /* GraphQL */ `  
  query ListOrganisations($input: ListOrganisationsInput) {
    listOrganisations(input: $input)
  }
`;
export const listUsers = /* GraphQL */ `
  query ListUsers($input: listUsersInput) {
    listUsers(input: $input)
  }
`;
export const listInternalUsers = /* GraphQL */ `
  query ListInternalUsers($input: listInternalUsersInput) {
    listInternalUsers(input: $input)
  }
`;
export const listExternalUsers = /* GraphQL */ `
  query ListExternalUsers($input: listExternalUsersInput) {
    listExternalUsers(input: $input)
  }
`;
export const listGamingCenter = /* GraphQL */ `
  query ListGamingCenter($input: ListGamingCenterInput) {
    listGamingCenter(input: $input)
  }
`;
export const listDevices = /* GraphQL */ `
  query ListDevices($input: ListDevicesInput) {
    listDevices(input: $input)
  }
`;
export const listSessions = /* GraphQL */ `
  query ListSessions($input: listSessionsInputs) {
    listSessions(input: $input)
  }
`;
export const listAvailableDevice = /* GraphQL */ `
  query ListAvailableDevice($input: listAvailableDeviceInput) {
    listAvailableDevice(input: $input)
  }
`;
